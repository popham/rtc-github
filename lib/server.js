...
var google = {
    name : 'google',
    initpoint : 'https://accounts.google.com/o/oauth2/auth',
    midpoint : 'https://accounts.google.com/o/oauth2/token',
    endpoint : 'https://www.googleapis.com/plus/v1/people/me',
    scopes : 'profile'
};

var facebook = {
    name : 'facebook',
    initpoint : 'https://www.facebook.com/dialog/oauth',
    midpoint : 'https://graph.facebook.com/oauth/access_token',
    endpoint : 'https://graph.facebook.com/v2.1/me',
    scopes : ''
};
...
var initiatorUrl = function (authenticator, client, csrf, email) {
    var query = {
        response_type : 'code',
        client_id : client.id,
        redirect_uri : [oauthServer.domain, authenticator.name, 'callback'].join('/'),
    ...
    return authenticator.initpoint + '?' + querystring.stringify(query);
};
...
var callbackCsrf = function (request, response, next) {
    ...
    if (oauthCsrf && oauthCsrf === request.query.state) {
        next();
    } else {
...
httpApp.get('/', function (request, response) {
    if (request.user && request.user.id) {
        ...
        context.logout = "(function () { window.parent.location='"+oauthServer.domain+"/logout'; return false; })();";
        response.render('status.jade', context);
    } else {
        var csrf = request.session.oauthCsrf = uuid.v4();
        response.render('login.jade', {
            facebook : login(initiatorUrl(facebook, facebookClient, csrf)),
            google : login(initiatorUrl(google, googleClient, csrf))
        });
    }
});

httpApp.get('/facebook/callback', function (request, response) {
    if (request.user) response.redirect(oauthServer.successUrl);
    if (!request.query.code) response.status(404).end();

    var get = {
        code : request.query.code,
        client_id : facebookClient.id,
        client_secret : facebookClient.secret,
        redirect_uri : [oauthServer.domain, facebook.name, 'callback'].join('/')
    };

    unirest.get(facebook.midpoint)
        .query(get)
        .end(function (response1) {
            if (response1.statusCode >= 400) {
                response.redirect(oauthServer.failureUrl);
                return;
            }

            response1 = querystring.parse(response1.body);

            unirest.get(facebook.endpoint)
                .header('Authorization', 'Bearer '+response1.access_token)
                ...
                .end(function (response2) {
                    ...
                    var profile = {
                        ...
                        displayName : response2.body.name
                    };

                    oauthUser(profile, function (error, user) {
                        if (error) {
                            response.redirect(oauthServer.failureUrl);
                        } else {
                            request.session.userId = user.id;
                            response.redirect(oauthServer.successUrl);
                        }
                    });
                });
        });
...
httpApp.get('/google/callback', function (request, response) {
    ...
    var post = {
        ...
        redirect_uri : [oauthServer.domain, google.name, 'callback'].join('/'),
        grant_type : 'authorization_code'
    };

    unirest.post(google.midpoint)
...
wssApp.ws('/', function(wss, request) {
    ...
    user.setId(userId);
    user.setName(name);
    wss.send(nonframed.fromStruct(root));

    // Provide the initial host list to the new connection holder.
    wss.send(nonframed.fromStruct(buildHostsUpdate()));

    wss.on('message', function (bytes) {
        var message = nonframed.toArena(bytes).getRoot(client.Client);
        ...
        case message.SERVICE:
            ...
            var update = nonframed.fromStruct(buildHostsUpdate());
        ...
        case message.PEER:
            ...
            if (target.isUserId() && targetSocket) {
                ...
                user.setId(userId);
                user.setName(name);
                targetSocket.send(nonframed.fromStruct(response));
            } // Else do nothing.
            break;
