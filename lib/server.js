...
var nonframe = require('capnp-js/nonframe');
...
wssApp.ws('/', function(wss, request) {
    ...
    user.setId(userId);
    user.setName(name);
    wss.send(nonframe.fromStruct(root));

    // Provide the initial host list to the new connection holder.
    wss.send(nonframe.fromStruct(buildHostsUpdate()));

    wss.on('message', function (bytes) {
        var message = nonframe.toArena(bytes).getRoot(client.Client);
        ...
        case message.SERVICE:
            ...
            var update = nonframe.fromStruct(buildHostsUpdate());
        ...
        case message.PEER:
            ...
            if (target.isUserId() && targetSocket) {
                ...
                user.setId(userId);
                user.setName(name);
                targetSocket.send(nonframe.fromStruct(response));
            } // Else do nothing.
            break;
